  ...
  <div class="app-container-padding">
    <el-form :model="queryParams" ref="queryForm" size="mini" :inline="true" class="iptAndBtn" style="position: relative">
      <div style="white-space: nowrap;margin-right: 90px;">
        <el-form-item label="出库日期" class="daterange">
          <el-date-picker
    ...
    <div class="btn">
      ...
      <el-dropdown>
        ...
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item @click.native="handleExport(0)"><i class="iconfont icon-daochuwenjian"></i>导出出库单</el-dropdown-item>
          <el-dropdown-item @click.native="handleExport(1)"><i class="iconfont icon-daochuwenjian"></i>导出出库单明细</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
...
import {warehouseRecord, warehouseDetailsView, delWarehouseDetails} from "@/api/accessories/accessoriesWarehouse";
...
export default {
  ...
  data(){
    return{
      ...
      tableDate: [],
      tableColumn: [
        {label: '出库单号', prop: 'operationNo', width: '120'},
        {label: '供应商', prop: 'millId', width: '120'},
        {label: '出库数量', prop: 'originalNum', width: '120'},
        {label: '出库金额（元）', prop: 'totalAmount', width: '120'},
        {label: '负责人', prop: 'person', width: '100'},
        {label: '出库日期', prop: 'operationDate', width: '120'},
        {label: '备注', prop: 'remarks', width: '100'},
      ],
      //总合计
      summation: [
        {label: 'originalNum', title: '出库总数量', value: 0,},
        {label: 'totalAmount', title: '出库总金额', value: 0, unit: '元'},
      ],
      detailDialog: false,
      detailsInfo: [
        {title: '出库单号',label:'operationNo', value: ''},
        {title: '供应商',label:'millId',  value: ''},
        {title: '出库时间', label:'operationDate', value: ''},
        {title: '出库人',label:'person',  value: ''},
      ...
      detailDialogColumns: [
        ...
        {label: '采购编号', prop: 'purchaseNo', width: '120'},
        {label: '辅料类型', prop: 'typeName', width: '120'},
        {label: '出库数量', prop: 'operationNum', width: '120'},
        {label: '出库单价（元/m²）', prop: 'operationPrice', width: '150'},
      ],
    }
  },
  created() {
  ...
  methods:{
    ...
    handleQuery() {
      ...
      warehouseRecord(this.queryParams).then(res => {
        ...
        this.queryParams.operationType ='出库';
    ...
    handleDetails() {
      ...
      if (this.selected.length == 1) {
        ...
        if (this.selected.length == 1) {
          ...
          this.detailsInfo[5].value = this.selected[0].totalAmount
    ...
    handleDelete() {
      if (!this.selected || this.selected === [] || this.selected.length === 0) {
        ...
        this.$confirm('此操作将删除选中数据，是否继续?', '提示', {
          ...
          delWarehouseDetails({ids: ids.toString(), type: 1}).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
                message: res.msg
