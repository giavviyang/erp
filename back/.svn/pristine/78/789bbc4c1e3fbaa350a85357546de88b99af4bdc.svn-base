    ...
    <el-dialog :title="millDialogType=='add'?'新增厂家':millDialogType=='edit'?'编辑厂家':''"
      ...
      <el-form :model="millForm" :rules="millFormRules" ref="millFormRef" class="ipt2 addMill">
        ...
        <el-form-item label="备注" prop="remarks" class="address">
          <el-input
            v-model="millForm.remarks" type="textarea" size="mini" placeholder="请输入内容">
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  data(){
    return{
      ...
      millList: [],         // 表格数据
      millListColumn: [
        {label: '厂家名称', prop: 'millName',width:'120'},
    ...
    getMillTypeList() {
      getMillTypeList().then(response => {
        if (response.code === 200) {
          // this.millTypeData = response.data;
