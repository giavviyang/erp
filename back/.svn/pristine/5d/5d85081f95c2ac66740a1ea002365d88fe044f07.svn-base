  ...
  <div class="app-container">
    <div class="containerTop">
      <div class="completionInfo">
        ...
        <el-form  size="mini" style="margin-top: 10px" :model="completionForm" class="oneForm" :inline="true" ref="ruleForm" label-width="50px" :rules="rules">
          <el-row>
            <el-col :span="12">
              <el-form-item label="工序" prop="completionCraftId">
                <el-select style="width: 100%" v-model="completionForm.completionCraftId" placeholder='请选择工序' prop="orderType">
                  <el-option
            ...
            <el-col :span="12">
              <el-form-item label="班组" prop="completionTeam">
                <el-select style="width: 100%" v-model="completionForm.completionTeam" placeholder='请选择班组' prop="team">
                  <el-option
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="班次" prop="completionShift">
                <el-select style="width: 100%" v-model="completionForm.completionShift" placeholder='请选择班次' prop="workShift">
                  <el-option
...
import {getFlowCardInfo,addCompletion} from "@/api/product/completion"
...
export default {
  ...
  async created() {
    ...
    if(this.$route.query.flowCardNo){
      this.flowCraftIpt=this.$route.query.flowCardNo;
      // let parms = {
      ...
      //   return false
      // }
      // let isRepeat = this.flowCraftList.filter(item=>{
      //   return item.flowCardNo == parms.flowCardNo
      // })
      // if(isRepeat.length <= 0){
      //   getFlowCardInfo(parms).then(res=>{
      //     this.flowCraftList = this.flowCraftList.concat(res.data.map(item=>{
      //       item.completeNum = item.completeNum ?? 0
      //       item.num = Number(item.itemNum) - Number(item.completeNum) - Number(item.damageNum)
  ...
  methods: {
    flowCardFun(e,index){
      let { value } = e.target
      let maxNum = (Number(this.flowCraftList[index].itemNum) - (Number(this.flowCraftList[index].completeNum)+Number(this.flowCraftList[index].damageNum)))
      let minNum = this.flowCraftList[index].completeNum
      if(value > maxNum){
        this.$message.warning("完工数量不能大于总数量")
        this.$set(this.flowCraftList[index],'num',maxNum)
        return false
      }
      if(value < minNum){
        this.$message.warning("完工数量不能小于0")
        this.$set(this.flowCraftList[index],'num',minNum)
        return false
      }
    ...
    getTeamList(){
      getAllTeam().then(res=>{
        ...
        this.completionForm.completionTeam = this.teamOptions[0].id
      })
    },
    /* 获取工序 */
    getCraftList() {
      getCraftList({pageNum : 1, pageSize: 10000}).then(res => {
        ...
        if (res.code === 200) {
          ...
          this.completionForm.completionCraftId = this.craftOptions[0].id
          this.completionForm.completionCraftName = this.craftOptions[0].craftName
        }
      })
      return false;
    },
      ...
      this.$refs.ruleForm.validate((valid)=> {
        if (valid) {
          ...
          submitForm.completionPersonList = this.completionPersonList.map(item=>{
            return{
              completionPerson:item.nickName,
              completionPersonId:item.userId,
              completionProportion:item.completionProportion,
              completionCoefficient:item.completionCoefficient
            }
          });
          submitForm.completionNum = 0
          submitForm.completionBusinessList = this.flowCraftList.filter(item=> {return item.num > 0}).map(item => {
          ...
          addCompletion(submitForm).then(res=>{
