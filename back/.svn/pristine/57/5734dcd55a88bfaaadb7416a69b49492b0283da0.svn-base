...
import {warehouseRecord, warehouseDetailsView, delInventoryDetails} from "@/api/original/warehouse";
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        {label: '盘库数量（片）', prop: 'originalNum', width: '120'},
      ...
      //总合计
      summation: [
        {label: 'originalNum', title: '盘库总数量', value: 0, unit: '片'},
        {label: 'originalArea', title: '盘库总面积', value: 0, unit: 'm²'},
        {label: 'totalAmount', title: '盘库总金额', value: 0, unit: '元'},
      ],
      detailDialog: false,
      detailsInfo: [
        {title: '盘库单号',label:'operationNo', value: ''},
      ...
      detailDialogColumns: [
        ...
        {label: '盘库数量（片）', prop: 'operationNum', width: '120'},
      ],
    }
  },
  created() {
   ...
   methods: {
    ...
    handleDetails() {
      ...
      if (this.selected.length == 1) {
        ...
        this.detailsInfo[1].value = this.selected[0].operationDate
