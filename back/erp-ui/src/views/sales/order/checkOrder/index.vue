<template>
  <div class="app-container-padding addOrder">
    ...
    <div class="countAndBtn">
      ...
      <div class="btn">
        <el-button
          ...
          size="mini"
          @click="handleSizeCheck">尺寸审核
        </el-button>
        <el-button
          type="primary"
          icon="el-icon-document-checked"
          size="mini"
          @click="handleCraftCheck">工艺审核
        </el-button>
        <el-button
          type="primary"
    ...
    <el-dialog
      title="尺寸审核"
      :visible.sync="sizeCheckDialog"
      width="80%"
      class="dialog-style sizeCheckDialog"
      ...
      <slot-table class="rightTable">
        <el-table highlight-current-row
          ...
          <el-table-column v-for="(item,index) in sizeCheckColumn"
                           :key="index"
                           :label="item.label"
                           :prop="item.prop"
            ...
            <template slot-scope="scope">
              <div v-if="scope.row[item.prop]!=scope.row['check'+item.prop]" class="sizeChecked">
                {{ scope.row[item.prop] }}
              </div>
              <div v-if="scope.row[item.prop]==scope.row['check'+item.prop]">{{ scope.row[item.prop] }}</div>
            </template>
          </el-table-column>
          <el-table-column v-for="(checkItem,checkIndex) in sizeCheckColumn2"
                           :key="'00'+checkIndex"
                           :label="checkItem.label"
                           :prop="checkItem.prop"
                           :min-width="checkItem.width"
                           v-if="checkItem.count">
            <template slot-scope="scope">
              <!--可根据类型来显示为搜索框、下拉框等-->
              <el-input
                v-model="scope.row[checkItem.prop]"
                size="mini"
                clearable/>
<!--              <el-input-number size="mini"  v-model="scope.row[checkItem.prop]" :min="0" :controls="false"></el-input-number>-->
<!--              <el-input-number size="mini"  v-model="scope.row[checkItem.prop]" ></el-input-number>-->
            </template>
          </el-table-column>
        </el-table>
      </slot-table>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="saveSizeCheck(0)" size="mini">尺寸审核通过</el-button>
         <el-button @click="saveSizeCheck(1)" size="mini">尺寸审核不通过</el-button>
      </span>
    </el-dialog>
    <!--  工艺审核弹窗  -->
    <el-dialog
      title="工艺审核"
      :visible.sync="craftCheckDialog"
      width="80%"
      class="dialog-style sizeCheckDialog"
      :close-on-click-modal="false"  :close-on-press-escape="false" :destroy-on-close="true">
      <slot-table class="rightTable">
        <el-table highlight-current-row
          ...
          height="100%"
          class="sizeCheckTable"
          style="width: 100%" slot="table"  :span-method="processSpanMethod">
          <el-table-column
            type="index"
            label="序号"
            width="50">
          ...
          <el-table-column
            v-for="(item,index) in craftCheckColumn" :key="index" :prop="item.prop" :label="item.label"
            v-if="item.count"
            :min-width="item.width">
          </el-table-column>
        </el-table>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="saveCraftCheck(0)" size="mini">工艺审核通过</el-button>
         <el-button @click="saveCraftCheck(1)" size="mini">工艺审核不通过</el-button>
      </span>
    </el-dialog>
    <!--  审核通过   -->
    <el-dialog
      ...
      :close-on-click-modal="false"  :close-on-press-escape="false" :destroy-on-close="true">
      <el-form ref="checkForm" :model="checkForm" :rules="checkRules" size="mini">
        <el-form-item label="审核人" prop="reviewedBy">
          <el-input v-model="checkForm.reviewedBy" clearable size="mini" placeholder="请输入审核人"/>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="saveCheck('checkForm')">确定</el-button>
    ...
    <el-dialog
      title="审核不通过"
      :visible.sync="failureCheckDialog"
      width="30%"
      class="dialog-style checkDialog"
      :close-on-click-modal="false"  :close-on-press-escape="false" :destroy-on-close="true">
      <el-form ref="failureCheckForm" :model="failureCheckForm" :rules="failureCheckRules" size="mini">
        <el-form-item label="审核人" prop="reviewedBy">
          <el-input v-model="failureCheckForm.reviewedBy" disabled size="mini" placeholder="请输入审核人"/>
        </el-form-item>
        <el-form-item label="不通过原因" prop="failureReason">
          <el-input v-model="failureCheckForm.failureReason" clearable type="textarea" size="mini" placeholder="请输入不通过原因"/>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="failureCheck('failureCheckForm')">确定</el-button>
        <el-button size="mini" @click="failureCheckDialog=false">取消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  created() {
   ...
   methods: {
      ...
      listOrder(this.queryParams).then(res => {
        ...
        if (res.code === 200) {
          ...
          this.summation.forEach(sumItem => {
            ...
            if (res.data[0].hasOwnProperty(sumItem.label)) {
    ...
    /* 尺寸审核 */
    handleSizeCheck() {
      this.sizeCheckDialog = true;
      let obj = {};
      listProduct({orderId: this.$route.params.id}).then(response => {
        if (response.data) {
          response.data.forEach(item => {
            this.sizeCheckColumn.forEach(columnItem => {
              if (item[columnItem.prop]) {
                columnItem.count++;
              }
            })
            this.sizeCheckColumn2 = JSON.parse(JSON.stringify(this.sizeCheckColumn.slice(0)));
            if (this.sizeCheckColumn2) {
              this.sizeCheckColumn2.forEach(item => {
                item.label = '确认' + item.label;
                item.prop = 'check' + item.prop;
                item.sizeCheck = 'sizeCheck' + item.prop;
                item.width = 30 + Number(item.width);
    ...
    /* 工艺审核 */
    handleCraftCheck() {
      this.craftCheckDialog = true;
      queryProcess({orderId: this.basicInfoForm.id}).then(res => {
        this.tableData = res.data;
        if (this.tableData) {
          this.tableData.forEach(item => {
            this.craftCheckColumn.forEach(columnItem => {
              if (item[columnItem.prop]) {
                columnItem.count++;
              }
            })
        ...
        for (var i = 0; i < this.tableData.length; i++) {
          if (i === 0) {
            ...
            if (this.tableData[i].productName === this.tableData[i - 1].productName &&
              this.tableData[i].itemWide === this.tableData[i - 1].itemWide &&
              this.tableData[i].itemHigh === this.tableData[i - 1].itemHigh) {
    ...
    handleCheckPass() {
      ...
      this.$confirm('是否确认审核通过, 是否继续?', '提示', {
        ...
        this.checkForm.reviewedBy = localStorage.getItem("nickName");
        this.saveCheck();
      }).catch(() => {});
    },
    /* 审核不通过 */
    handleNoPassCheck() {
      this.failureCheckDialog = true;
      this.failureCheckForm.reviewedBy = localStorage.getItem("nickName");
    },
    /* 确定审核通过 */
    saveCheck(formName) {
      // this.$refs[formName].validate((valid) => {
      //   if (valid) {
          this.checkForm.orderId = this.basicInfoForm.id;
          this.checkForm.orderResult = 0;
          auditOrder(this.checkForm).then(res => {
            if (res.code === 200) {
              this.$message({
                message: res.msg,
                type: 'success'
    ...
    /* 确定审核不通过 */
    failureCheck(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.failureCheckForm.orderId = this.basicInfoForm.id;
          this.failureCheckForm.orderResult = 1;
          auditOrder(this.failureCheckForm).then(res => {
            if (res.code === 200) {
              this.$message({
                message: res.msg,
                type: 'success'
