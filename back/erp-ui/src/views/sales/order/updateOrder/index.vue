...
<!--                      placeholder="请选择客户名称"/>-->
            <el-select ref="customerName"
                       v-model="basicInfoForm.customerName" @keyup.enter.native="selectBlur" filterable allow-create placeholder="请选择客户" @blur="selectBlur" clearable>
              <el-option
          ...
          <el-form-item label="项目名称" prop="customNo">
            <el-input v-model="basicInfoForm.entryName" placeholder="请输入项目名称" clearable/>
          </el-form-item>
          ...
          <el-form-item label="下单人" prop="preparer">
            <el-input v-model="basicInfoForm.preparer"></el-input>
          </el-form-item>
      ...
      <el-form :model="productForm" ref="productFormRef" label-width="120px" size="mini"
        ...
        <el-form-item label="产品类型" prop="typeId">
          <el-cascader
            ...
            :options="productTypeData" clearable>
          </el-cascader>
        </el-form-item>
...
import {keepThreeNum, keepOneNum, isNumber, sum} from "@/utils/order/order";
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        orderDeposit: [
          {validator: checkEmergencyFee, trigger: 'blur'}
        ],
        urgentCost: [
          {validator: checkEmergencyFee, trigger: 'blur'}
        ],
          ...
          if (this.additionCheckList.includes(item.id)) {
            ...
            if (strings[0] == "数量") {//计算方式为    数量*价格
              ...
              this.hotSettings.columns.splice(insertIndex + 1, 0,
                ...
                {
                  data: item.additionalAlias + "_allAmount",//附加项总金额
                  type: 'numeric', className: 'htCenter htMiddle'  //水平垂直居中
                },
              )
              this.hotSettings.colHeaders.splice(insertIndex + 1, 0,
                item.additionalName + "单价",
              ...
              this.hotSettings.columns.splice(insertIndex + 1, 0,
                {
                  ...
                  type: 'text',     className: 'htCenter htMiddle'  //水平垂直居中
                },
              )
              this.hotSettings.colHeaders.splice(insertIndex + 1, 0,
                item.value
      ...
      if (!productName.size) {  // }else {
        ...
        this.summation.forEach(item => {
          if (item.label2 == "productAmount") {
            ...
            console.log(hot.getDataAtProp(item.label2))
            item.value = keepThreeNum(sum(hot.getDataAtProp(item.label2)));
            count++
          }
        })
          ...
          if (!productName.size) {
            ...
            queryProduct({productName: [...productName].toString()}).then(res => {
              ...
              if (count) {
                ...
                update(this.basicInfoForm).then(res => {
                  if (res.code === 200) {
                    this.$message({
                      message: res.msg,
                      type: 'success'
