        ...
        <div class="deliverInfo">
          <el-form :model="deliverInfo" size="mini" :inline="true" ref="ruleForm" class="ipt2" >
            ...
            <el-form-item label="备注" class="remarks">
              <el-input v-model="deliverInfo.deliverRemarks" type="textarea" clearable size="mini"
                        placeholder="请输入内容"/>
            </el-form-item>
        ...
        <div class="deliverPerson">
          ...
          <slot-table class="shelfList">
            <el-table highlight-current-row
              ...
              <el-table-column label="铁架数量（个）" prop="frameNum" width="150">
              ...
              <el-table-column label="使用单价（元/个）" prop="framePrice" width="150">
                <template slot-scope="scope">
                  <!--可根据类型来显示为搜索框、下拉框等-->
                  <el-input-number
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-button
                    @click.native.prevent="deleteRow(scope.$index, shelfList)"
                    type="text"
                    size="mini">
                    移除
                  </el-button>
    ...
    <slot-table class="deliverList">
      ...
      >
        ...
        <el-table-column
          ...
          show-overflow-tooltip>
          <template slot-scope="scope">
            <el-input-number size="mini" v-model="scope.row.printPrice" :min="1" v-if="item.type==='iptNum'"
                             style="width: 150px"></el-input-number>
            <!--            <p v-if="item.type!=='iptNum'">-->
            <!--              <span v-if="scope.row[item.prop]==0">0</span>-->
            <!--              <span v-if="scope.row[item.prop]!=0">{{scope.row[item.prop]}}</span>-->
            <!--            </p>-->
            <template v-if="item.type!=='iptNum'">
              {{ scope.row[item.prop] }}
            </template>
          </template>
        </el-table-column>
        <el-table-column width="200" label="发货单价（元/m²）">
          <template slot-scope="scope">
            <el-input-number size="mini" v-model="scope.row.newUnitPrice" :min="0"></el-input-number>
          </template>
        </el-table-column>
        <el-table-column width="200" label="发货数量">
          <template slot-scope="scope">
            <el-input-number size="mini" v-model="scope.row.completionNum" :min="1" :max="scope.row.noShelfNum"
...
import {productionNumber, obtainOrderProduct, obtainPackProduct, saveDeliver, updateDeliver} from "@/api/salse/deliver";
...
export default {
  ...
  data() {
    return {
      deliverInfo: {
        deliverNo: '',
        customerName: '',
        deliverDate: getCurrentDay(),
        deliverMode: '',
        deliverPerson: localStorage.getItem("nickName"),
        deliverPhone: '',
        deliverAddress: '',
        shippingType: '',
        driver: '',
      ...
      formSize: '收起',   //表单最大化
      formFlag: false,
      selectShelfDialog: false,  //选择铁架弹窗
      shelfList: [],
      shelfListColumns: [
        {label: '铁架编号', prop: 'frameNo'},
        {label: '铁架名称', prop: 'frameName'},
        {label: '铁架规格', prop: 'frameSpecs'},
      ],
      deliverList: [],  //发货列表
      selected: [], //发货选择框
      deliverListColumns: [
        ...
        {label: '宽（mm）', prop: 'wideHead', type: 'ipt', width: '110'},
        {label: '高（mm）', prop: 'highHead', width: '110'},
        {label: '产品数量（片）', prop: 'num', width: '120'},
        // {label: '完工数量（片）', prop: 'completionNum', width: '120'},
        {label: '已发货数量（片）', prop: 'orderDeliverNum', width: '130'},
        {label: '可发货数量（片）', prop: 'noShelfNum', width: '130'},
      ],
      getType: '订单获取',
      getTypeDialog: false,
      customerNameList: [],
  ...
  //设置表格表体高度自适应
  updated() {
    if (this.getTypeDialog) {
      var multipleTable = this.$refs.multipleTable.$refs.bodyWrapper
      multipleTable.addEventListener('scroll', () => {
        // 滚动距离
        const scrollLeft1 = multipleTable.scrollLeft;
        this.$refs.multipleTable.$refs.headerWrapper.scrollLeft = scrollLeft1
      })
  ...
  methods: {
    ...
    productionNumber() {
      productionNumber().then(res => {
        this.deliverInfo.deliverNo = res.msg;
      })
    },
    /* 选择客户 */
    handleBlur() {
    ...
    saveSelectCustomer(val) {
      ...
      if (val.length === 1) {
        ...
        this.deliverInfo.deliverPhone = val[0].phone;
        ...
        if (res.code === 200) {
          ...
          this.shelfList.forEach(item => {
            for (let i = this.frameList.length - 1; i >= 0; i--) {
              if (item.frameId === this.frameList[i].id) {
    ...
    handleOrder() {
      ...
      this.getTypeColumns = [
        ...
        {label: '产品名称', prop: 'productName', width: '150', type: 'ipt'},
        ...
        {label: '产品面积（m²）', prop: 'productArea', width: '120'},
        {label: '产品重量（t）', prop: 'productWeight', width: '120'},
        {label: '完工数量（片）', prop: 'completionNum', width: '120'},
        {label: '入库数量（片）', prop: 'warehousingQuantity', width: '120'},
        {label: '打包数量（片）', prop: 'orderPackNum', width: '120'},
        {label: '已发货数量（片）', prop: 'orderDeliverNum', width: '120'},
        {label: '仓库位置', prop: 'warehouseLocation', width: '120'},
        {label: '铁架位置', prop: 'shelfLocation', width: '120'},
    ...
    handlePacking() {
      ...
      this.getTypeDateRange = [];
      this.getTypeColumns = [
        {label: '打包编号', prop: 'packNo', width: '140', type: 'ipt'},
        {label: '打包名称', prop: 'packName', width: '120', type: 'ipt'},
        {label: '订单编号', prop: 'orderNo', width: '120', type: 'ipt'},
        {label: '自定义编号', prop: 'customNo', width: '140', type: 'ipt'},
        {label: '客户名称', prop: 'customerName', width: '120', type: 'ipt'},
        {label: '项目名称', prop: 'entryName', width: '180', type: 'ipt'},
        {label: '产品名称', prop: 'productName', width: '150', type: 'ipt'},
        {label: '宽（mm）', prop: 'wideHead', width: '100'},
        ...
        // {label: '完工数量（片）', prop: 'completionNum', width: '120'},
        {label: '已发货数量（片）', prop: 'orderDeliverNum', width: '120'},
        {label: '可发货数量（片）', prop: 'noShelfNum', width: '120'},
      ];
      // this.deliverList = [];
    ...
    handleFlowCard() {
      ...
      this.getTypeDateRange = [];
      this.getTypeColumns = [
        {label: '流程卡号', prop: 'orderNo', width: '140', type: 'ipt'},
        {label: '订单编号', prop: 'orderNo', width: '140', type: 'ipt'},
        {label: '客户名称', prop: 'customerName', width: '120', type: 'ipt'},
        {label: '项目名称', prop: 'entryName', width: '180', type: 'ipt'},
        {label: '产品名称', prop: 'preparationDate', width: '150', type: 'date'},
        {label: '数量（片）', prop: 'preparer', type: 'ipt'},
        {label: '面积（m²）', prop: 'orderReview.reviewedBy'},
        {label: '分架日期', prop: 'orderReview.auditTime', width: '150'},
      ];
      this.getTypeList = [];
      customerList({
        pagenum: 1,
    ...
    /* 产品列表点击行 */
    handleDeliverRowClick(row, column) {
    ...
    /* 查询获取类型信息 */
    handleGetType() {
      if (this.deliverInfo.customerName) {
        ...
        if (keys.length !== 1) {
          ...
          for (let i = 0; i < this.getTypeSelected.length; i++) {
            ...
            for (let j = 0; j < this.deliverList.length; j++) {
              if (this.getTypeSelected[i].id == this.deliverList[j].id) {
                Message.warning("序号为" + (index + 1) + "的数据已经添加,不可重复添加！");
                return;
              }
