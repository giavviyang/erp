<template>
  <div class="app-container-padding">
    <el-form :model="queryParams" ref="queryForm" size="mini" :inline="true" class="iptAndBtn">
      ...
      <el-form-item label="负责人" prop="damageNo">
        <el-input
    ...
    <div class="btn">
      <el-button
        ...
        size="mini"
        @click="handleDetails"
        v-hasPermi="['originalFilm:damage:viewDetail']"
      >查看明细
      </el-button>
      <el-button
        type="primary"
        icon="el-icon-plus"
        size="mini"
        @click="handleAdd"  v-hasPermi="['originalFilm:damage:addDamageData']"
      >新增报损
      </el-button>
      <el-button
        type="primary"
        icon="el-icon-edit"
        size="mini"
        @click="handleUpdate"  v-hasPermi="['originalFilm:damage:updDamageData']"
      >编辑报损
      </el-button>
      <el-button
        type="primary"
        icon="el-icon-delete"
        size="mini"
        @click="handleDelete"  v-hasPermi="['originalFilm:damage:delDamageData']">删除报损
      </el-button>
      <el-button
        type="primary"
        icon="el-icon-printer"
        size="mini"
        @click="handlePrint"  v-hasPermi="['original:damage:print']">打印
      </el-button>
      <el-dropdown>
        <el-button type="primary" size="mini" v-hasPermi="['original:damage:exportDamageDetails']">
      ...
      <div class="addInfo">
        ...
        <div class="addInfoContainer">
          <el-form :model="addParams" ref="addForm" size="mini" :inline="true" class="ipt2">
            ...
            <el-form-item label="报损原因" prop="damageNo">
              <el-input
            ...
            <el-form-item label="备注" prop="remarks" class="remarks">
              <el-input
                type="textarea"
                v-model="addParams.remarks" size="mini" placeholder="请输入内容">
              </el-input>
            </el-form-item>
          </el-form>
        </div>
        ...
        <slot-table class="addTable">
          <el-table highlight-current-row
            ...
            <el-table-column
              ...
              :min-width="item.width"
              show-overflow-tooltip></el-table-column>
            <el-table-column label="库存数量（片）" width="120">
            ...
            <el-table-column label="报损数量（片）" width="150">
              <template slot-scope="scope">
                <el-input-number size="mini" v-model="scope.row.operationNum" :min="1" :precision="0"
                                 :max="Number(scope.row.stock)" style="width: 120px"></el-input-number>
              </template>
            </el-table-column>
            <el-table-column label="报损总金额（元）" width="150">
              <template slot-scope="scope">
...
import {delOriginalFilm} from "@/api/original/originalFilm";
import {productionNumber, queryDamageData, addDamageData, viewDetail,  updDamageDataQuery, updDamageData, delDamageData} from "@/api/original/originalDamage";
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        {label: '报损金额（元）', prop: 'damageAmount', width: '120'},
      ...
      //总合计
      summation: [
        {label: 'damageNum', title: '报损总数量', value: 0, unit: '片'},
        {label: 'damageArea', title: '报损总面积', value: 0, unit: 'm²'},
  ...
  created() {
   ...
   methods: {
    ...
    handleExport(param) {
      ...
      if (param === 0) {//导出订单
        this.download('original/damage/exportDamage', {
          ids: ids.toString()
        }, `报损单.xlsx`)
      } else if (param === 1) {//导出产品
        this.download('original/damage/exportDamageDetails', {
          ids: ids.toString()
        }, `报损单明细.xlsx`)
      }
    },
    ...
    saveAdd(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.originalFilmList.length == 0 || !this.originalFilmList || this.originalFilmList.length == undefined) {
            Message.warning("请添加原片信息");
          } else {
